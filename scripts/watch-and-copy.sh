...
echo "🔨 Building..."
  ...
  if pnpm --filter @digivice/game sync-assets && pnpm exec tsc -b && BUILD_FOR_FLUTTER=true NODE_ENV=development NATIVE_FEATURE_DEBUG_MODE=true APP_LOGO_TEXT="${APP_LOGO_TEXT:-DEBUG}" pnpm exec vite build --mode development 2>&1; then
    cd ../..
