...
const en = {
    ...
    "offlineAd.message": "Repeated ad connection failures may negatively affect monster growth.",
...
function getSupportedLocaleByCanonicalTag(languageTag) {
...
function resolveChineseLocale(parts) {
...
export function resolveLocaleFromLanguageTag(value) {
...
export function resolveLocaleFromLanguageTags(values, fallback = DEFAULT_LOCALE) {
...
export function assertTranslationParity() {
    ...
    for (const locale of SUPPORTED_LOCALES) {
        ...
        if (missingKeys.length > 0 || extraKeys.length > 0) {
            throw new Error(`[i18n] Locale ${locale} dictionary mismatch. missing=${missingKeys.join(",")}; extra=${extraKeys.join(",")}`);
        }
    }
}
