...
import { createEmptyMonsterBookState, getMonsterBookRecords } from "../monsterBook";
...
function createEvolutionTestContext(now = 50_000) {
  const world = createTestWorld({ now }) as ReturnType<typeof createTestWorld> & {
    getInMemoryData: () => any;
...
function mockPendingSpritesheetLoad() {
  ...
  const dummySpritesheet = Object.create(PIXI.Spritesheet.prototype) as PIXI.Spritesheet;
...
test("pending 중복 진화 요청은 1회만 기록된다", async () => {
  ...
  try {
    ...
    assert.equal(totalReachRecords, 1);
...
test("pending 진화 완료 전에 캐릭터가 바뀌면 stale completion을 버린다", async () => {
  ...
  try {
    ...
    assert.equal(CharacterStatusComp.characterKey[eid], CharacterKeyECS.GreenSlimeB1);
