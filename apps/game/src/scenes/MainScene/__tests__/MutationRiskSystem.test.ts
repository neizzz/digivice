...
test("성체 비질병 주사는 주사 스택을 최대 10까지 누적하고 클래스별 주기로 디톡스된다", () => {
  const injectionTime = 1_000;
  const world = createTestWorld({ now: injectionTime });
  const characterEid = withMockedDateNow(injectionTime, () =>
  ...
  mutationRiskSystem({
    world: world as any,
    currentTime: injectionTime + HOUR_MS,
  });

  assert.equal(MutationRiskComp.unnecessaryInjectionStacks[characterEid], 9);
});

test("legacy 주사 stack은 detox 기준 시각이 없으면 보존하고 현재 시각을 기준점으로 저장한다", () => {
  const currentTime = 5 * HOUR_MS;
  ...
  MutationRiskComp.unnecessaryInjectionStacks[characterEid] = 3;
  MutationRiskComp.lastInjectionDetoxTime[characterEid] = 0;
  ...
  assert.equal(MutationRiskComp.unnecessaryInjectionStacks[characterEid], 3);
  assert.equal(
    MutationRiskComp.lastInjectionDetoxTime[characterEid],
    currentTime,
  );
});
