...
test("상단 근처의 지속 상태 아이콘은 상단 경계 아래로 clamp된다", () => {
  withMockedStatusIconSprites(() => {
    ...
    const expectedMinY = world.positionBoundary.y + STATUS_ICON_SIZE / 2;

    assert.equal(sprite.x, 80);
    assert.ok(Math.abs(sprite.y - expectedMinY) < 0.000001);
  });
});

test("상단 근처의 일시 상태 아이콘은 상단 경계 아래로 clamp된다", () => {
  withMockedStatusIconSprites(() => {
    const world = createMainSceneWorldForTest();
    const eid = createTestCharacter(
      world as unknown as Parameters<typeof createTestCharacter>[0],
