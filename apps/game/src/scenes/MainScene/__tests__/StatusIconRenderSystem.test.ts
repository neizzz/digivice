...
test("TemporaryStatusComp에만 있는 happy도 emotion 아이콘으로 렌더링된다", () => {
  withMockedStatusIconSprites(() => {
    const world = createMainSceneWorldForTest();
    const eid = createTestCharacter(
      world as unknown as Parameters<typeof createTestCharacter>[0],
    ...
    TemporaryStatusComp.startTime[eid] = 1000;
...
test("TemporaryStatusComp에만 있는 discover도 emotion 아이콘으로 렌더링된다", () => {
  withMockedStatusIconSprites(() => {
    const world = createMainSceneWorldForTest();
    const eid = createTestCharacter(
      world as unknown as Parameters<typeof createTestCharacter>[0],
...
test("stage에서 분리된 기존 sick 아이콘은 다음 렌더에서 즉시 재부착된다", () => {
  withMockedStatusIconSprites(() => {
    const world = createMainSceneWorldForTest();
    const eid = createTestCharacter(
      world as unknown as Parameters<typeof createTestCharacter>[0],
...
test("persistent status와 TemporaryStatusComp emotion이 함께 있으면 status icon이 왼쪽부터 먼저 배치된다", () => {
  withMockedStatusIconSprites(() => {
    const world = createMainSceneWorldForTest();
    const eid = createTestCharacter(
      world as unknown as Parameters<typeof createTestCharacter>[0],
    ...
    assertApproximatelyEqual(sprites[0].y, STATUS_ICON_CENTER_Y_AT_CHARACTER_TOP);
    assertApproximatelyEqual(sprites[1].y, STATUS_ICON_CENTER_Y_AT_CHARACTER_TOP);
    assert.equal(StatusIconRenderComp.visibleCount[eid], 2);
  });
});
...
test("수면 중에는 TemporaryStatusComp emotion보다 sleeping 아이콘이 우선한다", () => {
  withMockedStatusIconSprites(() => {
    const world = createMainSceneWorldForTest();
    const eid = createTestCharacter(
      world as unknown as Parameters<typeof createTestCharacter>[0],
...
test("sick 상태로 수면 중이면 sick과 sleeping 아이콘이 함께 렌더링된다", () => {
  withMockedStatusIconSprites(() => {
    const world = createMainSceneWorldForTest();
    const eid = createTestCharacter(
      world as unknown as Parameters<typeof createTestCharacter>[0],
...
test("SleepFX 토글이 꺼져 있어도 sleeping 아이콘은 렌더링된다", () => {
  withMockedStatusIconSprites(() => {
    const world = createMainSceneWorldForTest();
    createTestCharacter(
      world as unknown as Parameters<typeof createTestCharacter>[0],
