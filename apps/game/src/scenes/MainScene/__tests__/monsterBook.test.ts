...
test("ensureMonsterBookBackfillFromSavedData는 현재 부화된 캐릭터만 1회 보정한다", () => {
  ...
  assert.equal(hasReachedMonster(state, CharacterKeyECS.TestGreenSlimeA1), true);
  assert.equal(hasReachedMonster(state, CharacterKeyECS.TestGreenSlimeB1), false);

  ensureMonsterBookBackfillFromSavedData(data, 3_000);
  const records = getMonsterBookRecords(state, CharacterKeyECS.TestGreenSlimeA1);
