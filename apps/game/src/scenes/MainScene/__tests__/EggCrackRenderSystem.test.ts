...
test("egg hatch duration과 syringeCount는 저장과 복원 시 round-trip된다", () => {
  ...
  const restoredWorld = createTestWorld({ now: 3_000 });
...
test("mixed clock egg schedule은 duration보다 큰 remaining을 현재 world time 기준으로 보정한다", () => {
  const world = createTestWorld({ now: 1_000_000 });
  const eid = addEntity(world);
  ...
  applySavedEntityToECS(world, eid, {
    components: {
      ...
      eggHatch: {
        hatchTime: 1_000_000 + 120 * 60 * 60 * 1000,
  ...
  assert.equal(EggHatchComp.hatchTime[eid], 1_005_000);
