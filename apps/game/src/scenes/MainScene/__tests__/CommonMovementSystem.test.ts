...
test("idle 캐릭터는 화면 상단에서 실제 실루엣 기준 14px까지만 더 올라갈 수 있다", () => {
  const world = createTestWorld({
    now: 0,
    positionBoundary: SMALL_BOUNDARY,
  });
  ...
  assert.equal(PositionComp.y[eid], 18);
  assert.equal(bounds.topY, -14);
  assert.equal(bounds.bottomY, 50);
});
...
test("화면보다 큰 캐릭터는 허용 경계가 붕괴하면 비대칭 overflow를 반영한 축 기준점으로 고정된다", () => {
  ...
  assert.equal(PositionComp.y[eid], 48);
});
