...
test("ReentrySimulator는 주입된 현재 시각으로 경과 시간을 계산한다", async () => {
  const world = createTestWorld({ now: 0, isSimulationMode: true });
  const simulator = new ReentrySimulator();
  const deltas: number[] = [];

  await withMockedDateNowAsync(999_999_999, async () => {
    await simulator.simulate(10_000, ({ delta }) => {
      deltas.push(delta);
    }, world, 13_500);
  });

  assert.equal(deltas.reduce((sum, delta) => sum + delta, 0), 3_500);
});
...
test("reentry는 egg hatch, 이동, urgent death 현재 동작을 함께 반영한다", async () => {
  ...
  try {
    await withMockedRandomAsync(0, () =>
      withMockedDateNowAsync(GAME_CONSTANTS.EGG_HATCH_MODE_TIME + 55_000, async () => {
...
test("reentry는 알 부화 시간 전이면 egg 상태와 위치를 유지한다", async () => {
  ...
  await withMockedRandomAsync(0, () =>
    withMockedDateNowAsync(GAME_CONSTANTS.EGG_HATCH_MODE_TIME - 1_000, async () => {
      await simulator.simulate(
        0,
        ({ world: simulationWorld, delta }) => {
          runSimulationStep({
...
test("reentry는 10초 tick 모바일 경계에서도 부화 후 위치를 갱신한다", async () => {
  ...
  try {
    await withMockedRandomAsync(0.25, () =>
      withMockedDateNowAsync(
        GAME_CONSTANTS.EGG_HATCH_MODE_TIME + 10 * 60_000,
        async () => {
          await simulator.simulate(
            0,
            ({ world: simulationWorld, delta }) => {
  ...
  assert.notEqual(ObjectComp.state[eggEid], CharacterState.EGG);
  assert.ok(
    Math.abs(PositionComp.x[eggEid] - initialEggX) > 0.001 ||
      Math.abs(PositionComp.y[eggEid] - initialEggY) > 0.001,
  );
...
test("reentry는 red stamina 감소 배율 기준으로 stamina가 0이 되면 urgent와 deathTime을 만든다", async () => {
  const world = createTestWorld({
    now: 0,
    isSimulationMode: true,
  });
  ...
  const simulator = new ReentrySimulator();
  const runSimulationStep = buildReentrySimulationStep(simulator);
  const redStaminaDecreaseElapsed =
    GAME_CONSTANTS.STAMINA_DECREASE_INTERVAL /
    GAME_CONSTANTS.LOW_STAMINA_DECAY_MULTIPLIER;

  await withMockedRandomAsync(0, () =>
    withMockedDateNowAsync(redStaminaDecreaseElapsed, async () => {
      await simulator.simulate(
        0,
        ({ world: simulationWorld, delta }) => {
          runSimulationStep({
  ...
  assert.equal(
    ...
    redStaminaDecreaseElapsed,
  );
  assert.equal(
    VitalityComp.deathTime[eid],
    redStaminaDecreaseElapsed + GAME_CONSTANTS.DEATH_DELAY,
  );
  assert.notEqual(ObjectComp.state[eid], CharacterState.DEAD);
});
