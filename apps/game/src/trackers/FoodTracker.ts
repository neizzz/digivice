...
export class FoodTracker {
  ...
  private isEnabled = false; // 트래커 활성화 여부
  ...
  public disable(): void {
    if (this.isEnabled) {
      ...
      // 현재 진행 중인 음식 처리가 있다면 취소
      if (this.isMovingToFood) {
        this.cancelCurrentFoodProcessing();
      }
    }
  }
   ...
   * 현재 진행 중인 음식 처리 취소
   */
  private cancelCurrentFoodProcessing(): void {
    if (this.foodQueue.length > 0 && this.isMovingToFood) {
      const currentFoodId = this.foodQueue[0];
      const food = this.findFoodById(currentFoodId);
  ...
  private setupEventListeners(): void {
    // Food가 착지했을 때 처리
    EventBus.subscribe(EventTypes.Food.FOOD_LANDED, (data) => {
      if (this.isEnabled) {
    ...
    // 음식을 다 먹었을 때 처리
    EventBus.subscribe(EventTypes.Food.FOOD_EATING_FINISHED, (data) => {
   ...
   */
  private _startFoodSearchInterval(): void {
    // 이미 인터벌이 실행 중이면 중단하고 새로 시작
    if (this.searchInterval !== null) {
      clearInterval(this.searchInterval);
    }

    // 비활성화 상태면 인터벌을 시작하지 않음
    if (!this.isEnabled) {
      return;
    }
   ...
   * 음식을 검색하고 처리하는 메서드
   */
  private async _searchAndProcessFood(): Promise<void> {
    // 비활성화 상태이거나 이미 검색 중이거나 이동/먹기 중이면 무시
    if (
      !this.isEnabled ||
  ...
  private async processNextFood(): Promise<void> {
    // 비활성화 상태이거나 대기열이 비어있거나 이미 처리 중이면 무시
    if (!this.isEnabled || this.foodQueue.length === 0 || this.isMovingToFood) {
      return;
    }
    ...
    try {
      ...
      // 비활성화 상태가 아니고, 다음 음식이 있고, 현재 먹는 중이 아니라면 다음 음식 처리
      if (
        this.isEnabled &&
  ...
  private async moveCharacterToFood(food: Food): Promise<void> {
    return new Promise<void>((resolve) => {
      ...
      const moveToFoodTick = (deltaTime: number) => {
        // 트래커가 비활성화되었거나 디버그 플래그 확인
        if (!this.isEnabled || DebugFlags.getInstance().isEatingPrevented()) {
