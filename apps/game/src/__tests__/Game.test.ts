...
import type { SceneRenderTimingSample } from "../interfaces/Scene";
...
test("renderer render 이후 onFrameRenderTiming hook으로 프레임 계측 샘플을 전달한다", () => {
  const receivedSamples: SceneRenderTimingSample[] = [];
  const frameAwareScene = {
    onFrameRenderTiming: (sample: SceneRenderTimingSample) => {
      receivedSamples.push(sample);
    },
  };

  const fakeGame = {
    currentScene: frameAwareScene,
    _pendingSceneRenderTiming: {
      scene: frameAwareScene,
      deltaTimeMs: 16.7,
      tickerGapMs: 17.1,
      updateStartedAtMs: 100,
      updateEndedAtMs: 104.2,
      sceneUpdateCostMs: 4.2,
    },
    _isFrameTimingAwareScene: Game.prototype["_isFrameTimingAwareScene"],
  };

  Game.prototype["_handleRendererRender"].call(
    fakeGame as unknown as Game,
    105.1,
    107.6,
  );
  ...
  assert.equal(receivedSamples[0]?.timestampMs, 107.6);
  assert.equal(receivedSamples[0]?.deltaTimeMs, 16.7);
  assert.equal(receivedSamples[0]?.tickerGapMs, 17.1);
  assert.equal(receivedSamples[0]?.sceneUpdateCostMs, 4.2);
  assert.ok(
    Math.abs((receivedSamples[0]?.updateToRenderStartMs ?? 0) - 0.9) < 0.000001,
  );
  assert.ok(Math.abs((receivedSamples[0]?.renderCostMs ?? 0) - 2.5) < 0.000001);
  assert.ok(
    Math.abs((receivedSamples[0]?.frameEndToEndCostMs ?? 0) - 7.6) < 0.000001,
  );
  assert.equal(fakeGame._pendingSceneRenderTiming, null);
});
...
test("initialize는 devicePixelRatio를 최대 렌더 해상도 2로 clamp한다", async () => {
  ...
  const fakeGame: any = {
    ...
    _isPixiReady: false,
    _installRendererPerfHooks: () => undefined,
    _onResize: () => undefined,
