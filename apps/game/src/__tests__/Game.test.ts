...
test("changeScene는 loading 중 인터럽트 요청이 오면 이전 scene으로 복귀한다", async () => {
  ...
  const fakeGame: {
    ...
    preloadSceneAssets: (key: SceneKey) => Promise<void>;
    ...
    preloadSceneAssets: async () => undefined,
    _createScene: async () => {
      createSceneCalled += 1;
      return {} as never;
    },
...
test("changeScene는 FlappyBird 전환 시 기존 scene exit 전에 preload를 완료한다", async () => {
  const events: string[] = [];
  ...
  const fakeGame: {
    ...
    preloadSceneAssets: (key: SceneKey) => Promise<void>;
    _createScene: (key: SceneKey) => Promise<typeof createdScene>;
    app: {
    ...
    _onSceneTransitionStateChange: (params: {
      ...
      events.push(params.state);
    },
    preloadSceneAssets: async (key) => {
      events.push(`preload:${key}`);
    },
    _createScene: async (key) => {
  ...
  const originalRemoveChildren = stage.removeChildren.bind(stage);
  stage.removeChildren = (...args) => {
    events.push("remove_children");
    return originalRemoveChildren(...args);
  };
  ...
  assert.deepEqual(events, [
    "loading",
    "preload:flappy_bird_game",
    "exit",
    "remove_children",
    "create:flappy_bird_game",
    "destroy_previous",
    "core_ready",
  ]);
...
test("changeScene는 FlappyBird preload 중 인터럽트되면 기존 scene을 유지한다", async () => {
  const states: string[] = [];
  let onSceneExitCalled = 0;
  let onSceneReenterCalled = 0;
  ...
  assert.equal(changed, false);
  assert.deepEqual(states, ["loading", "interrupted"]);
  assert.equal(onSceneExitCalled, 0);
  assert.equal(onSceneReenterCalled, 1);
...
test("getWorldDataSyncPayload는 MainSceneWorld일 때만 live export를 전달한다", () => {
  const worldData = {
    world_metadata: {
      name: "MainScene",
      last_ecs_saved: 100,
  ...
  mainScene.buildWorldDataSyncPayload = () => worldData;

  const result = Game.prototype.getWorldDataSyncPayload.call({
...
test("getWorldDataSyncPayload는 MainSceneWorld가 아니면 null을 반환한다", () => {
  const result = Game.prototype.getWorldDataSyncPayload.call({
