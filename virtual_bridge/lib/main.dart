...
class WebView extends StatelessWidget {
  ...
  void _handleNfcRead(JavaScriptMessage message) async {
    ...
    await _log('_handleNfcRead raw message: "$serializedMessage"');
  ...
  void _handleNfcWrite(JavaScriptMessage message) async {
    ...
    await _log(
        '_handleNfcWrite message: "$serializedMessage"/$serializedJsArgs');
    _nfcController.startWriting(
        ...
        onWritten: ({required String writtenMessage}) {
          ...
          _log('NFC Write2: $writtenMessage');
          _resolvePromise(
              id: (jsArgs as JavaScriptInterfaceArgsMapType).id,
              data: writtenMessage);
        });
  }
